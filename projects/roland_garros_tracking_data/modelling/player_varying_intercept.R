...
options(mc.cores = 4)
player_varying_intercept_model <- stan_model('./modelling/stan_files/predict_player_intercept_2.stan')
...
wta_player_df %>%
  arrange(desc(median_wide)) %>% View()

wta_player_df %>%
  arrange(desc(median_body)) %>% View()



# -- Save Data
plot_data <- rbind(atp_player_df, wta_player_df)
write.csv(plot_data, 'plot_median_probs_dataframe.csv', row.names = FALSE)
...
plot_data <- read.csv('./modelling/plot_median_probs_dataframe.csv')
...
ggplot(data = plot_data %>% 
  ...
  geom_label_repel(data = wta_plot_data %>%
                   ...
                   point.padding = unit(0.5, "lines"),
                   label.padding = unit(0.25, "lines")) +
  labs(title = 'WTA Estimated Serve Direction Probabilities',
    x='Average % Serving T',
    y = 'Average % Serving Wide',
    caption="Roland Garros 2019-20"
  ) +
  scale_y_continuous(breaks = c(0.2, 0.3, 0.4, 0.5))+
  peter_theme(family_font = 'Tahoma') 

ggsave('./modelling/wta_comparing_player_probs2.jpg',
       width=5.5, height=4,
       dpi = 280)
...
ggplot(data = atp_plot_data,
  ...
  geom_label_repel(data = atp_plot_data %>%
                   ...
                   point.padding = unit(0.5, "lines"),
                   label.padding = unit(0.25, "lines")) +
  labs(title = 'ATP Estimated Serve Direction Probabilities',
       x='Average Prob(Serve T)',
       y = 'Average Prob(Serve Wide)',
       caption="Roland Garros 2019-20"
  ) +
  geom_label_repel(data = atp_plot_data %>%
                     filter(name =='R.NADAL'),
...
ggsave('./modelling/atp_comparing_player_probs.jpg',
       width=5.5, height=4,
       dpi = 280)
