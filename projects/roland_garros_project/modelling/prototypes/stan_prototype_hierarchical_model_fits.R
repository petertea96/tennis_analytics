...
top_ID_T <- order(post_means_T, decreasing = TRUE)[1:5]
atp_training_data %>%
...
# x6 = returner_backhand_locT,
# x7 = point_importance,
# x8 = server_handL,
# x9 = court_side_ad,
# x10 = interaction_s_hand_court_side,
# x11 = interaction_ss_bh_T,

# T parameter plots
bayesplot::mcmc_hist(stanfit4, 
                     pars = var_names[rep(c(TRUE, FALSE), times = 11)]
                      )

# Wide parameter plots
bayesplot::mcmc_hist(stanfit4, 
                     pars = var_names[rep(c(FALSE, TRUE), times = 11)]
)
...
wta_model1 <- cmdstan_model('./modelling/stan_files/loglik_stan/loglik_model_1.stan')
wta_model2 <- cmdstan_model('./modelling/stan_files/loglik_stan/loglik_model_2.stan')
wta_model3 <- cmdstan_model('./modelling/stan_files/loglik_stan/loglik_model_3.stan')	
wta_model4 <- cmdstan_model('./modelling/stan_files/loglik_stan/loglik_model_4.stan')

# -- ADVI version the fastest MCMC to fit
fit_wta_model1 <- wta_model1$variational(
  data = wta_stan_datalist1,
  tol_rel_obj = 0.001
)

fit_wta_model2 <- wta_model2$variational(
  data = wta_stan_datalist2,
  tol_rel_obj = 0.001
)

fit_wta_model3 <- wta_model3$variational(
  data = wta_stan_datalist3,
  tol_rel_obj = 0.001
)

fit_wta_model4 <- wta_model4$variational(
...
# -- Convert object types to stanfit
stanfit1 <- rstan::read_stan_csv(fit_wta_model1$output_files())
stanfit2 <- rstan::read_stan_csv(fit_wta_model2$output_files())
stanfit3 <- rstan::read_stan_csv(fit_wta_model3$output_files())
stanfit4 <- rstan::read_stan_csv(fit_wta_model4$output_files())

# -- Save model fits
saveRDS(stanfit1, file = "./modelling/saved_models/advi/wta_loglik_fit1.RDS")
saveRDS(stanfit2, file = "./modelling/saved_models/advi/wta_loglik_fit2.RDS")
saveRDS(stanfit3, file = "./modelling/saved_models/advi/wta_loglik_fit3.RDS")
saveRDS(stanfit4, file = "./modelling/saved_models/advi/wta_loglik_fit4.RDS")
...
comp <- loo::loo_compare(loo_1, loo_2, loo_3, loo_4)
print(comp)
# elpd_diff se_diff
# model4     0.0       0.0
# model3  -331.5      26.2
...
top_ID_T
# S.KENIN, M.LINETTE, P.HERCOG, O.JABEUR, S.WILLIAMS
...
wta_training_data %>%
  ...
  distinct() %>%
  filter(server_index %in% top_ID_Wide )
# J.KONTA; S.WILLIAMS; S.KENIN; Ka.PLISKOVA; N.OSAKA; 
